	...
	if($generate_billet){
		...
		*/
		if($trans_id and !$error){
			...
			if(!$barcode and ($chargeExistID === $trans_id and ($chargeExistStatus === 'canceled' || $chargeExistStatus === 'unpaid'))){
				...
				if( is_int($charge_id) ){
					...
					if( is_array($pay_charge) ){
						...
						$addtrans = ggnb_add_trans($user_id,$invoice_id,$charge_id, $api_mode, $system_url);
						if($addtrans['error']){
			...
			///
			if(!$error and $chargeExistID === $trans_id and $chargeExistDuedate >= date('Y-m-d') and $chargeExistTotal !== $invoice_amount ){
				if($cancelBillet){
		...
		if( !$error and $redirectToBillet and stripos($_SERVER['REQUEST_URI'], 'viewinvoice.php') ){
			header_remove();
