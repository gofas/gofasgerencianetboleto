...
function gofasgerencianetboleto_link($params){
	...
	logModuleCall("gofasgerencianetboleto","genarate_billet_start",array('module_version'=>'3.2.1','params'=>$params,'user_id'=>$user_id),"", $vars);
	...
	if($generate_billet){
		// Verify Database
		 ...
		 if($debug_or_log){
			 $log_result['ggnb_verifyInstall']	= $ggnb_verifyInstall;
		}
		...
		*/
		if( $trans_id and !$error){
			...
			if($debug_or_log ){
				$log_result['chargeExist'] = $chargeExist_;
				$log_result['chargeExistTotal__']	= '$chargeExistTotal: '.$chargeExistTotal.'<br>$invoice_amount : '. $invoice_amount ;
				$log_result['chargeExistTotal_']	.= '<br>$discount_tax_valueRS: '.$discount_tax_valueRS.'<br>';
				if($chargeExistTotal === $invoice_amount ){
					$log_result['chargeExistTotal']	.= 'valor da transação existente é igual ao valor da fatura';
				}
				if($chargeExistTotal !== $invoice_amount ){
					$log_result['chargeExistTotal']	.= '<br>valor da transação existente é diferente do valor da fatura';
				}
			}
			...
			elseif(!$error and  ($chargeExistID === $trans_id and $chargeExistStatus !== 'canceled' and $chargeExistDuedate < date('Y-m-d')) and !$configurations and !$cancelBillet ){
				...
				if($debug_or_log){
					$log_result['updateBillet']	= $updateBillet;
				}
			}
			elseif(!$error and ($chargeExistID === $trans_id and $chargeExistStatus === 'canceled' and $chargeExistDuedate >= date('Y-m-d')) ){
				...
				if($debug_or_log){
					$log_result['create_charge_1']	= $create_charge;
				}
				...
				if( is_int($charge_id) ){
					...
					if($debug_or_log){
						$log_result['pay_charge_1']	= $pay_charge_;
					}
					...
					if( is_array($pay_charge) ){
						...
						$addtrans = ggnb_add_trans($user_id,$invoice_id,$charge_id,(int)$params['admin'], $api_mode, $system_url, $debug_or_log );
						if($addtrans['error']){
							$error	.= $addtrans['error'];
						}
						if($debug_or_log){
							$log_result['addtrans']	= $addtrans;
						}
						// Save billet on DB
						$ggnb_store_billet = ggnb_store_billet($pay_charge,$invoice_amount,$invoice_id,$debug_or_log,$api_mode);
						if($debug_or_log){
							$log_result['ggnb_store_billet']	= $ggnb_store_billet;
						}
			...
			elseif(!$error and $chargeExistID === $trans_id and $chargeExistDuedate >= date('Y-m-d') and $chargeExistTotal !== $invoice_amount ){
				if($cancelBillet){
					$cancelCharge = ggnb_cancel_charge($api_url,$access_token,$trans_id);
					 ...
					 if($debug_or_log){
						 $log_result['cancelCharge']	= $cancelCharge;
					 }
				}
				...
				$charge_id = $create_charge['result'];
				if($debug_or_log){
					$log_result['create_charge_2']	= $create_charge;
				}
				if($create_charge['error']){
					if($create_charge['error']['error_description']['property']){
						$error	.= $create_charge['error']['error_description']['property'].' '.$create_charge['error']['error_description']['message'];
				...
				if( is_int($charge_id) ){
					...
					$pay_charge		= $pay_charge_['result'];
					if($debug_or_log){
						$log_result['pay_charge_2']	= $pay_charge_;
					}
					if($pay_charge_['error']){
						$error	.= $pay_charge_['error'];
						//$log_result['pay_charge_2_error']	= $pay_charge_;
					}
					if( is_array($pay_charge) ){
						$link		= $pay_charge['data']['link'];
						$expire_at	= $pay_charge['data']['expire_at'];
			...
			elseif(!$error and ($chargeExistID === $trans_id and $chargeExistDuedate < date('Y-m-d') ) and ($configurations or $cancelBillet) ){
				...
				if( $chargeExistStatus === 'new' || $chargeExistStatus === 'waiting' || $chargeExistStatus === 'unpaid'){
					...
					if( $cancelCharge['result'] === 'success'){
						...
						$charge_id = $create_charge['result'];
						if($debug_or_log){
							$log_result['create_charge_3']	= $create_charge;
						}
						if($create_charge['error']){
							if($create_charge['error']['error_description']['property']){
								$error	.= $create_charge['error']['error_description']['property'].' '.$create_charge['error']['error_description']['message'];
				...
				elseif($chargeExistStatus === 'canceled'){ // ignora transação cancelada
					$create_charge = ggnb_create_charge( $api, $body, $system_url, $debug); // body
					$charge_id = $create_charge['result'];
					if($debug_or_log){
						$log_result['charge_id_']	= $create_charge;
					}
					if($create_charge['error']){
						$error	.= $create_charge['error'];
					}
				...
				if( is_int($charge_id) ){
					...
					$pay_charge		= $pay_charge_['result'];
					if($debug_or_log){
						$log_result['pay_charge_3']	= $pay_charge_;
					}
					if($pay_charge_['error']){
						$error	.= $pay_charge_['error'];
						//$log_result['pay_charge_3_error']	= $pay_charge_;
					}
					if( is_array($pay_charge) ){
						$link		= $pay_charge['data']['link'];
						$expire_at	= $pay_charge['data']['expire_at'];
		...
		elseif( !$trans_id ){
			...
			if( (float)$invoiceTotal >= (float)$minimunAmount){
				...
				$charge_id = $create_charge['result'];
				if($debug_or_log){
					$log_result['create_charge_4']	= $create_charge;
					$log_result['create_charge_4_error']	= $create_charge['error'];
				}
				if($create_charge['error']){
						$error	.= $create_charge['error'];
				}
				...
				if( is_int($charge_id) ){
					...
					$pay_charge		= $pay_charge_['result'];
					if($debug_or_log){
						$log_result['pay_charge_4']	= $pay_charge_;
					}
					if($pay_charge_['error']){
						$error	.= $pay_charge_['error'];
						//$log_result['pay_charge_4_error']	= $pay_charge_;
					}
					if( is_array($pay_charge) ){
						$link		= $pay_charge['data']['link'];
						$expire_at	= $pay_charge['data']['expire_at'];
		...
		if( $error ){
			...
			if( $emailonError ){
				$sendEmailonError = ggnb_send_error_email( $invoice_id, $user_id, $firstname, $lastname, $system_url, (int)$params['admin'], $emailonError, $error, $debug);
				if($debug_or_log){
					$log_result['sendEmailonError']	= $sendEmailonError;
 				}
			}
