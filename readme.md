...
Gerencianet agora é Efí. O nome mudou mas este continua sendo um dos módulos mais completos do mercado para emissão de boleto bancário registrado via WHMCS, integrando geração, consulta e baixa automática de boletos pela API Efí (antiga Gerencianet). Desenvolvido pela Gofas Software.

## Funcionalidades

- Emissão de boletos registrados via API Efí, sem taxas de registro/baixa
- 2ª via automática com valor atualizado (cálculo de multa e juros) ao acessar a fatura
- Confirmação e baixa automática de pagamento via notificações (callback) ou tarefa cron
- Cancelamento e recriação automática de boletos vencidos
- Juros e multa após o vencimento
- Desconto ou taxa adicional para pagamento via boleto, com validade configurável
- Desconto personalizado por cliente via campo personalizado do WHMCS
- Informações do boleto no corpo dos emails de fatura (link, linha digitável, vencimento e mais) via mergetags
- Linha digitável na fatura, com opção de copiar em um clique
- Anexo do boleto em PDF ao email de fatura (opcional)
- Substituir o link da fatura pelo link do boleto nos emails
- Redirecionamento direto para o boleto ao acessar a fatura
- Boleto com sua marca: logotipo, mensagem e instruções personalizadas
- Notificação por email ao suporte quando ocorre erro ao gerar o boleto
- Suporte a múltiplas contas e a ambientes de produção e de testes (sandbox)
